// Package sqlate is the session layer of the sqlate library: the one surface
// every authored statement runs through. It wraps a plain *sql.DB behind the
...
// above it — query, migrate — take a Session and never a driver.
//
// # Wrapper
//
...
// connections. It performs no I/O: a nil pool or dialect is a wiring defect
// and panics. [DB.Conn] pins one connection for a protocol that needs
// session scope, such as a session-level lock or a run of non-transactional
// DDL; [DB.Dialect] returns the dialect statements are compiled against.
//
// # Sessions and transactions
//
// [Session] is the querying surface the query and migrate packages take,
...
// spells the nth bind parameter, and how its driver's errors classify. An
...
// driver's error stays recoverable. The four constraint classes —
...
// a dialect's MapError, carrying the constraint name when the driver
// exposes it. sql.ErrNoRows is never mapped; it flows to the boundary
// unchanged.
package sqlate
